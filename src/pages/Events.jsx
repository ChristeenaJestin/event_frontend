    ...
    <DashboardLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8 }}>
        <div>
          ...
          <h1 className="h1" style={{ fontSize: 28, marginTop: 8 }}>Explore events</h1>
        </div>
        {canCreate && (
      ...
      <FilterSidebar active={category} onChange={setCategory} />

      {loading && <Loader label="Loading events…" />}
      {!loading && error && <EmptyState message="Couldn't load events. Please try again." />}
      {!loading && !error && events.length === 0 && <EmptyState message="No events found." />}
      {!loading && !error && events.length > 0 && (
        <EventGrid
          events={events}
          onEventClick={(e) => navigate(`/events/${e.id}`)}
          onCreateClick={canCreate ? () => navigate('/create-event') : undefined}
